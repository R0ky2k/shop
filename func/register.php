...
$log_text = "\n-------------- $date -----------------\n";
file_put_contents('log.txt', $log_text, FILE_APPEND);
...
$sql = $pdo->prepare("SELECT id FROM users WHERE login=:login OR password=:password OR email=:email");
...
if (testPassword($password, $password2)) {
    if ($array["id"] == 0) {
        $sql = $pdo->prepare("INSERT INTO users (admin, login, password, email) VALUES (0,:login, :password, :email)");
        $sql->execute(array("login" => $login, "password" => $password, "email" => $email));
